...
def test_get_latest_price_invalid_symbol():
    ...
    assert response.status_code == 404 or response.status_code == 500  # API might throw either
...
def test_poll_prices_empty_symbols():
    ...
    response = client.post("/prices/poll", json=payload)
    assert response.status_code == 422  # Unprocessable Entity due to empty list
